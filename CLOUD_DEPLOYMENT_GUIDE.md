...
除了数据库数据，各个服务的**配置**（如 AI 工作流、IoT 流程图）也需要迁移到云端环境。

### 1. n8n 工作流迁移

n8n 的工作流存储在 SQLite 数据库中（默认情况），最简单的迁移方式是**导出/导入 JSON**。

#### 导出（本地）
1. 打开本地 n8n 界面 (`http://localhost:5678`)。
2. 进入 **Workflows** 列表。
3. 选择要迁移的工作流 -> 点击右上角菜单 -> **Download**。
4. 保存为 `.json` 文件。
5. **凭证 (Credentials)**：
   - n8n 提供了导出凭证的功能（CLI），但在云端重新输入 API Key 通常更安全且不容易出错。
...
4. 选择之前导出的 `.json` 文件。
5. 打开导入的工作流，检查节点配置（特别是数据库连接节点），确保凭证已正确绑定。

### 2. Open WebUI 迁移

Open WebUI 主要依赖**环境变量**配置，聊天记录建议重新开始或使用内置导出功能。

#### 配置迁移
请确保将本地 `docker-compose.yml` 中的关键环境变量复制到 Railway 服务变量中：
- `OPENAI_API_BASE_URLS`
- `OPENAI_API_KEYS` (即 Gemini API Key)
- `ENABLE_RAG_WEB_SEARCH`

#### 数据迁移（聊天记录）
1. **本地**：点击左下角头像 -> **Settings** -> **Account** -> **Export All Data** (导出所有数据)。
2. **云端**：登录 Railway 部署的 Open WebUI -> **Settings** -> **Account** -> **Import Data**。
   > **注意**：如果不保留历史对话，直接在云端创建新账号使用即可。

### 3. Node-RED 迁移

Node-RED 的核心是 `flows.json` 文件。

#### 导出（本地）
1. 打开本地 Node-RED (`http://localhost:1880`)。
2. 点击右上角菜单 (三横线) -> **Export**。
3. 选择 **All flows** (所有流程)。
4. 点击 **Download** 保存为 `.json` 文件。
...
4. 点击 **Import**。
5. **检查节点配置**：
   - 特别注意 **InfluxDB Out** 节点和 **TCP/MQTT** 节点。
   - 确保 InfluxDB 连接配置已更新为云端地址（或 Railway 内部地址）。
6. 点击右上角 **Deploy** 生效。
...
如果您将来需要为客户交付这套系统，最优雅的方式是提供**一键部署 (One-Click Deploy)**。

### 方案一：Railway Templates (推荐)

Railway 允许您将整个项目配置（包含所有服务、变量连接、启动命令）打包成一个模板链接。
...
3. **获取部署链接**：
   ...
   - 或者生成一个 Markdown 按钮代码。

#### 客户体验
客户只需点击您提供的链接：
1. 这里会显示所有包含的服务列表。
2. 客户登录 GitHub 和 Railway。
3. **关键点**：Railway 会自动提示客户输入必要的变量（如 API Keys），而像数据库连接字符串这种内部变量会自动处理。
4. 点击部署，几分钟后一套完整的私有环境就搭建完成了。

### 方案二：VPS 一键安装脚本

对于拥有自己服务器的客户，可以封装一个 Shell 脚本。
