...
- `logic-engine/`: IEC 81346编码解析与拓扑分析服务（Python FastAPI）。
...
此文件定义了完整的开发环境，包括PostgreSQL、InfluxDB、n8n、Node-RED、Logic Engine、Grafana和Open WebUI等服务。
...
4.  **验证Logic Engine服务**:
    访问 `http://localhost:8000/health` 验证IEC 81346编码解析服务是否正常运行。
...
**解决方案**: 检查`3001`（后端）、`5173`（前端）、`5432`（PostgreSQL）、`8000`（Logic Engine）、`8086`（InfluxDB）、`5678`（n8n）、`1880`（Node-RED）等端口是否被其他程序占用。您可以使用`lsof -i :3001`（macOS/Linux）或`netstat -ano | findstr :3001`（Windows）查找占用进程并终止，或修改`.env`文件中的端口号。
