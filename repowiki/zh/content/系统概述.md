...
- [src/services/api/facilities.ts](file://src/services/api/facilities.ts)
- [src/types/facility.ts](file://src/types/facility.ts)
</cite>
...
TwinSight数字孪生平台旨在构建一个集成化的设施管理中枢，通过融合设施管理、3D可视化、资产管控、文档智能分析与时序数据监控五大核心能力，实现物理空间与数字世界的无缝连接。其核心价值主张在于打破传统设施管理中信息孤岛的壁垒，为设施管理者提供一个统一、直观、智能的操作界面。
...
Atomic API 是 TwinSight 为 AI Hub 提供的专用 M2M（Machine-to-Machine）API 接口层，位于 `/api/atomic/v1/` 路径下。它采用双重认证机制（JWT + Service Token）和 Scope 作用域隔离，为 AI Agent 提供标准化的数据查询和控制能力。

**核心端点：**
- `/assets/query` - RDS 资产层级查询
- `/timeseries/query` - 时序数据统一查询
- `/power/trace` - 电源拓扑追溯
- `/knowledge/rag-search` - RAG 知识库检索
- `/ui/command` - UI 控制指令（导航、高亮、隔离等）
- `/alarm/create` - 报警事件创建

**安全特性：**
- 服务间认证（X-Service-Token）
- 用户 JWT 透传
- Scope 作用域隔离（X-Project-Id、X-File-Id）
- 审计日志追踪
...
WebSocket 控制通道服务（路径：`/ws/control`）是实现 AI Hub 到前端实时指令推送的核心组件。基于 Socket.IO 构建，提供双向实时通信能力，使 AI Agent 能够直接控制前端 3D 查看器的行为。

**核心特性：**
- **JWT 握手鉴权**：连接时验证用户身份
- **Room 隔离机制**：基于用户、项目、会话的多维度隔离
- **禁止全局广播**：仅允许定向 Room 推送，确保安全
- **动态项目切换**：支持运行时切换项目 Room

**典型使用场景：**
AI Hub 分析发现某设备异常，通过 Atomic API 的 `/ui/command` 端点发送高亮指令，WebSocket 服务将指令推送到运维人员的前端界面，自动高亮显示异常设备并弹出分析报告。
...
TwinSight平台的核心功能围绕五大支柱展开：

1.  **设施管理**: 平台以设施（Facility）为顶层组织单元，支持多设施的创建、编辑和归档。每个设施可关联多个BIM模型文件，形成"设施→模型→空间/资产"的层级结构。通过`/api/v1/facilities` API，支持设施封面上传、状态管理和模型关联查询。

2.  **3D可视化**: 平台集成了Autodesk Forge Viewer，支持RVT、IFC等BIM模型的在线浏览。用户可以在3D模型中直观地查看建筑结构、设备位置和空间布局，实现物理空间的数字映射。

3.  **资产管控**: 提供对设施内所有资产（如设备、家具）的全生命周期管理。通过`/api/v1/assets`等API，支持资产的增删改查、批量导入、与BIM模型中的构件（dbId）关联，并支持按MC编码、分类、位置等维度进行管理。

4.  **文档智能分析**: 平台能够关联PDF、图片等文档，并利用AI技术进行智能分析。文档按设施隔离，当系统检测到异常（如温度超标）时，可自动触发AI工作流，分析相关文档，为运维人员提供处置建议和知识支持。

5.  **时序数据监控**: 集成InfluxDB时序数据库，用于存储和查询来自IoT传感器的实时与历史数据（如温度、湿度）。前端通过时间轴控件，支持对历史数据进行回溯分析，并能基于阈值触发预警。
...
- **Logic Engine**: 基于FastAPI构建的IEC 81346编码解析与拓扑分析服务，用于处理电气设备的层级编码和电源拓扑追溯。
...
1.  **多设施统一管理**: 管理员通过设施列表页面创建和管理多个建筑设施，为每个设施上传封面图片、设置状态，并关联对应的BIM模型。运维人员可通过设施卡片快速进入3D查看器，查看特定设施的资产和空间数据。
2.  **智能预警与故障诊断**: 当机房温度超过28°C时，系统自动触发AI分析工作流。Gemini模型结合设备手册、维护记录等文档，分析可能的故障原因（如冷却系统故障），并生成包含处置步骤和预防建议的报告，推送给运维人员。
3.  **资产全生命周期管理**: 新设备到货后，管理员将其信息录入系统，并与BIM模型中的对应构件关联。后续的维护记录、巡检报告、相关图纸均可在资产详情页中一站式查看。
4.  **历史数据回溯分析**: 运维人员发现某区域近期能耗异常，可通过时间轴控件回溯过去30天的能耗数据，结合天气、人员活动等数据，分析能耗升高的根本原因。
