...
### facilities (设施表)
存储设施（建筑/园区）的基本信息，作为模型文件的顶层组织单元。

**字段定义:**
- `id`: SERIAL, 主键, 自增
- `facility_code`: VARCHAR(100), 非空, 唯一
- `name`: VARCHAR(200), 非空
- `description`: TEXT
- `address`: VARCHAR(500)
- `cover_image_path`: VARCHAR(1000)
- `status`: VARCHAR(20), 默认值 `'active'`
- `metadata`: JSONB, 默认值 `'{}'`
- `created_at`: TIMESTAMP, 默认值 `CURRENT_TIMESTAMP`
- `updated_at`: TIMESTAMP, 默认值 `CURRENT_TIMESTAMP`

**索引:**
- `idx_facilities_code` on `facility_code`
- `idx_facilities_status` on `status`

**约束:**
- 主键: `id`
- 唯一约束: `facility_code`
- 非空: `facility_code`, `name`
...
facilities ||--o{ model_files : "1 to many"
...
facilities ||--o{ documents : "1 to many"
```
...
### 设施模型 (facility.js)
`facility.js`文件定义了与`facilities`表交互的DAO。作为模型文件的顶层组织单元，设施模型提供了完整的CRUD操作以及聚合查询功能。`getAllFacilities`和`getFacilityById`方法返回设施基本信息的同时，通过子查询聚合关联的模型数量、视图数量、资产数量、空间数量、IoT配置数量和文档数量。`getFacilityWithModels`方法进一步返回设施关联的所有模型文件及其视图信息。

设施编码采用 `FAC-{规范化名称}-{时间戳+随机码}` 的格式生成，确保全局唯一性。
