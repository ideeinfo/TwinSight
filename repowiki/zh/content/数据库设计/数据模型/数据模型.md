...
### 设施表 (facilities)
存储设施（建筑/园区）的基本信息，作为模型文件的顶层组织单元，形成"设施→模型→空间/资产"的层级结构。

| 字段名 | 数据类型 | 主键 | 外键 | 非空 | 默认值 | 说明 |
|--------|----------|------|------|------|--------|------|
| id | SERIAL | 是 | 否 | 是 | 无 | 自增主键 |
| facility_code | VARCHAR(100) | 否 | 否 | 是 | 无 | 设施编码，全局唯一 |
| name | VARCHAR(200) | 否 | 否 | 是 | 无 | 设施名称 |
| description | TEXT | 否 | 否 | 否 | 无 | 设施描述 |
| address | VARCHAR(500) | 否 | 否 | 否 | 无 | 设施地址 |
| cover_image_path | VARCHAR(1000) | 否 | 否 | 否 | 无 | 封面图片路径 |
| status | VARCHAR(20) | 否 | 否 | 否 | active | 状态：active/inactive |
| metadata | JSONB | 否 | 否 | 否 | {} | 扩展元数据 |
| created_at | TIMESTAMP | 否 | 否 | 否 | CURRENT_TIMESTAMP | 创建时间 |
| updated_at | TIMESTAMP | 否 | 否 | 否 | CURRENT_TIMESTAMP | 更新时间 |

**表级约束**:
- 唯一约束：`facility_code` 确保设施编码全局唯一
- 非空约束：`facility_code`, `name`
- 索引：`idx_facilities_code` (facility_code), `idx_facilities_status` (status)

**关联关系**:
- 一对多：`facilities` → `model_files`（一个设施可包含多个模型文件）
- 一对多：`facilities` → `documents`（一个设施可包含多个文档）
...
### 设施模型 (facility.js)
设施模型是系统中管理`facilities`表的数据访问层，实现了设施级别的资源组织和权限隔离。该模型提供了完整的CRUD操作以及聚合统计功能：

- **编码生成**：`generateFacilityCode`方法根据设施名称生成唯一的设施编码，格式为`FAC-{规范化名称}-{时间戳+随机码}`
- **聚合查询**：`getAllFacilities`和`getFacilityById`方法通过子查询返回关联的统计信息（模型数量、视图数量、资产数量、空间数量、IoT配置数量、文档数量）
- **模型关联**：`getFacilityModels`获取设施关联的所有模型文件，`getFacilityWithModels`进一步返回每个模型包含的视图列表
- **更新策略**：`updateFacility`方法使用动态字段映射，仅更新允许的字段，并自动维护`updated_at`时间戳

设施模型作为模型文件的顶层组织单元，支持"设施→模型→空间/资产"的层级数据隔离，是多租户场景下的重要数据边界。

**关键映射关系**:
- `facilityCode` -> `facility_code`
- `coverImagePath` -> `cover_image_path`
- `facilityId` -> `facility_id`（在documents和model_files表中）

**图示来源**
- [facility.js](file://server/models/facility.js#L1-L297)
...
同时，`documents.facility_id`通过外键关联`facilities`表，当设施被删除时，其关联的文档记录也会被清理。

这种设计确保了模型文件或设施删除时，其所有相关数据（资产、空间、配置、文档等）都能被彻底清理，避免了孤儿数据的产生。
...
- `facilities.facility_code`：确保设施编码全局唯一
...
TwinSight平台的数据模型以`facilities`（设施）为顶层组织单元，以`model_files`为核心表构建，形成了"设施→模型→空间/资产"的层级结构。数据库设计充分考虑了数据完整性，通过外键级联删除和唯一性约束保障了数据的一致性。模型层通过直接SQL操作实现了高效的数据库交互，同时在应用逻辑中实现了复杂的业务规则，如默认视图管理、映射配置的原子更新、设施级别的资源聚合统计等。整个数据架构支持了平台的核心功能，包括多设施BIM模型数据管理、文档关联、时序数据集成和用户权限控制。
