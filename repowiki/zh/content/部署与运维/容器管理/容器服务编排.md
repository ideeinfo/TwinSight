...
### Logic Engine - IEC 81346编码解析服务

`logic-engine` 服务是基于Python FastAPI构建的编码解析与拓扑分析引擎，支持IEC 81346标准编码的解析、验证和拓打分析。

**关键配置项**：
- **构建方式**：通过 `./logic-engine/Dockerfile` 本地构建
- **端口映射**：`8000:8000`
- **数据库连接**：使用拆分的数据库变量（`DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASSWORD`, `DB_NAME`）
- **健康检查**：HTTP检查 `/health` 端点

**核心功能**：
- IEC 81346 资产编码解析与验证
- 编码拓扑结构分析
- 与PostgreSQL数据库集成，支持资产数据查询

**服务来源**
- [docker-compose.yml](file://docker-compose.yml#L101-L122)
