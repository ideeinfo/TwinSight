...
系统采用Pinia作为状态管理解决方案，实现了模块化的状态管理架构。核心store模块包括认证(auth)、主题(theme)、用户界面(ui)、空间(spaces)、资产(assets)、IoT(iot)和模型(models)等，每个模块都有明确的职责划分。

设施（Facility）相关的状态管理采用轻量级方式：设施数据通过 `src/services/api/facilities.ts` API 服务直接获取，在 `FacilitiesView.vue` 和 `HomeDashboardView.vue` 等组件内以局部响应式状态管理，未引入独立的 Pinia store。认证 store 中的 `checkAuth` 方法在应用初始化时加载用户信息和权限，为设施访问控制提供基础。
...
各store模块之间保持松耦合，通过明确的接口进行交互。同时，store与服务层和组件层有清晰的依赖关系。设施相关组件（FacilitiesView、HomeDashboardView）直接调用 `api/facilities.ts` 服务获取数据，不经过 Pinia store。
...
L --> G
L --> H
FA[FacilitiesView.vue] --> AL[api/facilities.ts]
HD[HomeDashboardView.vue] --> AL
```
