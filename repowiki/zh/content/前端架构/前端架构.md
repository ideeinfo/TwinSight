...
路由系统由 `vue-router` 实现，配置文件位于 `src/router/index.js`。当前定义了八条路由，分为公开路由和需要认证的受保护路由：

| 路径 | 组件 | 需认证 | 说明 |
|------|------|--------|------|
| `/` | `HomeView.vue` | 否 | Landing 页，已认证用户自动重定向到 `/home` |
...
路由采用懒加载语法 `import()`，实现了代码分割，优化了首屏加载性能。全局前置守卫 `router.beforeEach` 实现认证拦截：已认证用户访问 `/` 自动重定向到 `/home`，未认证用户访问受保护路由时重定向到 `/`。
...
**Section sources**
- [router/index.js](file://src/router/index.js#L1-L81)
...
- [ManageView.vue](file://src/views/ManageView.vue)
...
具体的 API 模块（如 `assets.ts`、`spaces.ts`、`facilities.ts`）导入 `http.ts` 中的 `get`、`post` 等方法，定义具体的端点和参数。当前 API 模块包括：
- `assets.ts`：资产 CRUD 与批量导入
- `spaces.ts`：空间管理
- `models.ts`：模型文件管理
- `timeseries.ts`：时序数据查询
- `facilities.ts`：设施管理（封面上传、模型关联等）

`api/index.ts` 作为入口文件，统一导出所有 API 模块。这种分层设计使得业务组件无需关心底层网络细节，只需调用相应的服务方法即可，极大地提高了代码的可测试性和可维护性。

**Section sources**
- [services/api/index.ts](file://src/services/api/index.ts#L1-L15)
- [services/http.ts](file://src/services/http.ts#L1-L220)
- [services/api/facilities.ts](file://src/services/api/facilities.ts)

## 自定义组合函数（Composables）

组合函数（Composables）是 Vue 3 组合式 API 的核心优势，用于封装和复用有状态的逻辑。项目在 `src/composables/` 目录下定义了多个自定义组合函数。

- `useAppState.ts`：管理全局应用状态，包括面板可见性、加载状态等。
- `useViewer.ts`：封装 Forge Viewer 的初始化、模型加载、选择事件等核心功能。
- `useIoTData.ts`：处理 IoT 时序数据的获取、缓存和状态管理。
- `useSelection.ts`：管理模型构件的选择状态，支持单选和多选。
...
- [composables/useViewer.ts](file://src/composables/useViewer.ts)
- [composables/useIoTData.ts](file://src/composables/useIoTData.ts)
- [composables/useSelection.ts](file://src/composables/useSelection.ts)
...
C --> H[stores/theme.ts]
C --> I[stores/ui.ts]
C --> J[stores/models.ts]
C --> K[stores/assets.ts]
C --> L[stores/spaces.ts]
C --> M[stores/iot.ts]
D --> N[router/index.js]
N --> O[HomeView.vue]
N --> HD[HomeDashboardView.vue]
N --> FA[FacilitiesView.vue]
N --> MV[ManageView.vue]
N --> P[AppViewer.vue]
N --> Q[ThemeDebugPage.vue]
N --> R[ChartView.vue]
P --> S[TopBar]
P --> T[IconBar]
P --> U[LeftPanel]
...
P --> AE[useAppState]
AA --> M
AB --> AF[ViewerInstance]
P --> AG[http.ts]
AG --> AH[api/assets.ts]
AG --> AI[api/spaces.ts]
AG --> AJ[api/models.ts]
AG --> AK[api/timeseries.ts]
AG --> AL[api/facilities.ts]
```
