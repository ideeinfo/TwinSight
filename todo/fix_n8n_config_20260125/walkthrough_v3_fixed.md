...
### KB Sync Fix (New)
Updated `server/routes/files.js` to fix the clean-up order during KB reconstruction (`force=true`):
*   **Reordered Deletion**: Explicitly deletes records from `kb_documents` *before* deleting the parent `knowledge_bases` record.
*   **Orphan Cleanup**: Adds a fallback deletion query using the Open WebUI UUID to ensure any orphaned records (where the internal foreign key might be missing or broken) are also removed.
