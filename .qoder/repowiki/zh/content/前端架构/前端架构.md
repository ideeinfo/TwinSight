...
路由系统由 `vue-router` 实现，配置文件位于 `src/router/index.js`。当前定义了五个主要路由：
- 根路径 `/` 导向 `HomeView.vue`
- `/viewer` 和 `/assets` 路径导向 `AppViewer.vue`
- `/theme-debug` 导向 `ThemeDebugPage.vue`（主题调试页面）
- `/chart-view` 导向 `ChartView.vue`（图表独立视图）

路由采用懒加载语法 `import()`，实现了代码分割，优化了首屏加载性能。
...
- [ThemeDebugPage.vue](file://src/views/ThemeDebugPage.vue)
